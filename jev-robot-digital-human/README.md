...
- **对话界面**：右下角悬浮对话舱，展示回复、工具调用轨迹，一键下发指令
...
3. **切换引擎**：`自动`（真实 Jev 优先，失败回退本地）/ `真实 Jev` / `本地`
4. **调节参数**：决策间隔（800–6000ms）、置信度门控阈值（0–0.95）
5. **触发场景事件**：点击事件按钮改变感知状态，下一次决策即响应
6. **切换数字人**：`真实·机器人`（RobotExpressive）/ `真实·Xbot`
7. **骨骼调试**：`显示骨骼` 叠加 SkeletonHelper；`重置绑定姿态` 复位动画
