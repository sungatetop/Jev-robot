...
panel.cbOnAvatarMode = (mode) => setAvatarMode(mode);
...
function setAvatarInfo(mode) {
  const el = document.getElementById('avatar-info');
  if (el) el.textContent = AVATAR_CONFIG[mode]?.label || mode;
}
...
async function setAvatarMode(mode) {
  ...
  if (avatar.whenReady) {
    try {
      ...
      setAvatarInfo(mode + ' · 加载失败: ' + e.message);
      return;
    }
  }
  setAvatarInfo(mode);
  // 立即应用当前状态
  applyProxy.current({ motion: 'idle', expression: 'neutral', intensity: 1, lookAtUser: false });
}
...
function initFallback() {
  ...
  const avatar = stageEl.querySelector('.fb-avatar');
...
applyProxy.current = (decision) => {
  ...
  } else if (renderer && renderer.apply) {
    // 2D 兜底
